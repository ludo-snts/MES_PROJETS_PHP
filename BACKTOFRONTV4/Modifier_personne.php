...
<div id="containeurcrea">
    <div class="divheader" id="headercrea"><?php echo $langArray['modification'] ?></div>
    ...
    <form class="formcrea" method="POST" action="Controleur.php?action=creer_personne">
        ...
        <?php $personne = $_SESSION['personne']; ?>

        <div class="labelinput">

            <p>
                <label class="label" for="prenom"><?php echo $langArray['prenom'] ?> : </label>
                <input class="input" type="text" id="prenom" name="prenom" placeholder="<?php echo $langArray['enter'] . $langArray['prenom']; ?>" size='20' value='<?php if (isset($personne)) echo $personne->getPrenom(); ?>'>
                <img src="asset/png/champ.png" alt="astérisque rouge">
            </p>
            <p>
                <label class="label" for="nom"><?php echo $langArray['nom'] ?>: </label>
                <input class="input" type="text" id="nom" name="nom" placeholder="<?php echo $langArray['enter'] . $langArray['nom']; ?>" size='20' value='<?php if (isset($personne)) echo $personne->getNom(); ?>'>
                <img src="asset/png/champ.png" alt="astérisque rouge">
            </p>
            <p><label class="label" for="mail"><?php echo $langArray['mail'] ?>: </label>
                <input class="input" type="email" id="mail" name="mail" placeholder="<?php echo $langArray['enter'] . $langArray['mail']; ?>" size='20' value='<?php if (isset($personne)) echo $personne->getMail(); ?>'>
                <img src="asset/png/champ.png" alt="astérisque rouge">
            </p>
            <p>
                <label class="label" for="age"><?php echo $langArray['age'] ?>: </label>
                <input class="input" type="number" id="age" name="age" placeholder="<?php echo $langArray['enter'] . $langArray['age']; ?>" size='20' value='<?php if (isset($personne)) echo $personne->getAge(); ?>'><img src="asset/png/champ.png" alt="astérisque rouge">
            </p>
            <p><label class="label" for="username"><?php echo $langArray['user'] ?>: </label>
                <input class="input" type="text" id="username" name="username" placeholder="<?php echo $langArray['enter'] . $langArray['user']; ?>" size='20' value='<?php if (isset($personne)) echo $personne->getUsername(); ?>'>
                <img src="asset/png/champ.png" alt="astérisque rouge">
            </p>
            <p><label class="label" for="password"><?php echo $langArray['pass'] ?>: </label>
                <input class="input" type="text" id="password" name="password" placeholder="<?php echo $langArray['enter'] . $langArray['pass']; ?>" value='<?php if (isset($personne)) echo $personne->getPassword(); ?>'>
                <img src="asset/png/champ.png" alt="astérisque rouge">
            </p>
            <p>
                <label><?php echo $langArray['genre'] ?> : </label>
                ...
                <select class="select" name="genre" id="combobox" style="width: 149px">
                    ...
                    <?php foreach ($tab_genres as $genre) { ?>if ($langArray['id_genre']== 1)
                        <option value="<?php echo $genre->getId(); ?>">
                            <?php if (strcmp($genre->getLibelle(), "Masculin"))
                                echo $langArray['fe'];
                            else
                                echo $langArray['ho'];  ?></option>
                    <?php } ?>
                </select>
                <img src="asset/png/champ.png" alt="astérisque rouge">
            </p>
